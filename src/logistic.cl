...
__kernel void cost_kernel(__global float *o, const __global float *y, __global float *sum, int dim, Costfn c){
  agnostic_cost_kernel(o, y, sum, dim, c);
}

__kernel void cost_gradient_kernel(__global float *o, const __global float *y, __global float *dest, Costfn c){
  const int i = get_global_id(0);
