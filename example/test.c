...
int main(){
	...
	/* MLP tests */
	{
		...
		if(success)
			printf("  | TEST SUCCESSFUL: Default logistic functions were as expected.\n");
		...
		if(!assert_equal(tmp, l0, n.layers[0].size)){
			...
			printf("  | TEST SUCCESSFUL: First hidden layer's output was as expected.\n");
		}
		dispose_array(tmp);
		...
		if(!assert_equal(tmp, l1, n.layers[1].size)){
			...
			printf("  | TEST SUCCESSFUL: Second hidden layer's output was as expected.\n");
		}
		dispose_array(tmp);
		...
		if(!assert_equal(tmp, l2, n.layers[2].size)){
			...
			printf("  | TEST SUCCESSFUL: Third hidden layer's output was as expected.\n");
		}
		dispose_array(tmp);
		...
		if(!assert_equal(n.cost_gradient, c, n.output_dimension)){
			...
			printf("  | TEST SUCCESSFUL: Cost gradient was as expected.\n");
		}
		...
		if(!assert_equal(tmp, g0, n.layers[0].input_dimension)){
			...
			printf("  | TEST SUCCESSFUL: First hidden layer's input gradient was as expected.\n");
		}
		dispose_array(tmp);
		...
		if(!assert_equal(tmp, g1, n.layers[1].input_dimension)){
			...
			printf("  | TEST SUCCESSFUL: Second hidden layer's input gradient was as expected.\n");
		}
		dispose_array(tmp);
		...
		if(!assert_equal(tmp, g2, n.layers[2].input_dimension)){
			...
			printf("  | TEST SUCCESSFUL: Third hidden layer's input gradient was as expected.\n");
		}
		dispose_array(tmp);
		...
		if(!assert_equal(tmp, pg, n.num_params)){
			...
			printf("  | TEST SUCCESSFUL: Network parameter gradients were as expected.\n");
		}
	...
	/* LSTM tests */
	{
		...
		float l1_t3[] = {0.0515, 0.0854, 0.0790, 0.2063, 0.0369, 0.0437};
		...
		if(!assert_equal(tmp, l1_t1, n.layers[0].size)){
			printf("X | TEST FAILED: First hidden LSTM layer output was incorrect.\n");
		}else{
			printf("  | TEST SUCCESSFUL: First hidden LSTM layer output was as expected.\n");
		}
		dispose_array(tmp);

		tmp = retrieve_array(n.layers[1].output[0], n.layers[1].size);
		if(!assert_equal(tmp, l2_t1, n.layers[1].size)){
			printf("X | TEST FAILED: Second hidden LSTM layer output was incorrect.\n");
		}else{
			printf("  | TEST SUCCESSFUL: Second hidden LSTM layer output was as expected.\n");
		}
		dispose_array(tmp);

		tmp = retrieve_array(n.output_layer.output, n.output_dimension);
		if(!assert_equal(tmp, l3_t1, n.output_dimension)){
			printf("X | TEST FAILED: LSTM output layer output was incorrect.\n");
		}else{
			printf("  | TEST SUCCESSFUL: LSTM output layer output was as expected.\n");
		}
		dispose_array(tmp);
		...
		for(int i = 0; i < n.depth; i++){
			printf("layer %d t 1\n", i+1);
			PRINTLIST(n.layers[i].output[0], n.layers[i].size);
		}
		printf("layer 3 t 1\n");
	...
	//float *PRINTLIST(n.layers[0].input_gradient, n.layers[0].input_dimension);
/*
