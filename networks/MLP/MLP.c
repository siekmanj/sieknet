...
static Layer *create_layer(size_t size, Layer *previousLayer){
  ...
  for(int i = 0; i < size; i++){
    ...
    for(int j = 0; j < previous_layer_size; j++){
      weights[j] = ((float)(rand()%70)-33)/10; //Slight bias towards positive weights
    }
...
void addLayer(MLP *n, size_t size){
  ...
  if(n->output != NULL) n->output->squish = sigmoid;
  n->output = newLayer;
  n->output->squish = softmax;
...
/*
 * Description: Performs the feed-forward operation on the network.i
